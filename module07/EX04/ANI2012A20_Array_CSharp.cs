// ANI2012A20_Array_CSharp.cs | Programmation C# avec Unity
// Exemples de tableaux avec 1, 2 ou 3 dimensions.

using System;
using UnityEngine;

public class ANI2012A20_Array_CSharp : MonoBehaviour
{
  void Start()
  {
    int indexX = 0;
    ...
    for (indexX = 0; indexX < countX; ++indexX)
      print("la valeur à l'index [" + indexX + "] est " + array1D[indexX]);
    ...
    for (indexX = 0; indexX < countX; ++indexX)
    {
      for (indexY = 0; indexY < countY; ++indexY)
        print("la valeur à l'index [" + indexX + "][" + indexY + "] est " + array2D[indexX, indexY]);
    }
    ...
    for (indexX = 0; indexX < countX; ++indexX)
    {
      for (indexY = 0; indexY < countY; ++indexY)
      {
        for (indexZ = 0; indexZ < countZ; ++indexZ)
          print("la valeur à l'index [" + indexX + "][" + indexY + "][" + indexZ + "] est " + array3D[indexX, indexY, indexZ]);
      }
    }
  }
}
